...
# --- Image Manipulation (Pure Function) ---
...
def save_image(image: Image.Image, source_path: str) -> bool:
    try:
...
def process_image(image_path: str) -> bool:
    ...
    match load_result:
        case ImageLoadResult(success=True, image=loaded_im, warning=warn_msg):
            if warn_msg:
                logger.warning(f"{warn_msg} - Image: {image_path}")
            logger.info(f"Successfully loaded image {image_path}: {loaded_im.size}")
            image_to_process = loaded_im
        case ImageLoadResult(success=False, error=err):
            ...
            return False
        case _:
            logger.error(f"Unexpected result type from load_convert_image for {image_path}")
            return False

    # Step 2: Extract Dominant Colors
    color_result = extract_dominant_colors(image_to_process)
    match color_result:
        case ColorResult(success=True, colors=palette):
             if not palette:
                  logger.error(f"Color extraction succeeded but returned no palette for {image_path}")
                  return False
             logger.debug(f"Extracted palette for {image_path} (Top 1: {palette[0]})")
             background_color = palette[1] if len(palette) > 1 else palette[0]
        case ColorResult(success=False, error=err):
            ...
            return False
        case _:
            logger.error(f"Unexpected result type from extract_dominant_colors for {image_path}")
            return False

    # Step 3: Generate Final Image (using the pure function)
    try:
        final_image = generate_final_image(image_to_process, background_color)
    except Exception as e:
    ...
    # Step 4: Save Result
    if save_image(final_image, image_path):
        return True
    else:
        return False
